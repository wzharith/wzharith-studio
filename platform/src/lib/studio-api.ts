/**
 * Studio API client — all persistence goes through Neon Postgres via `/api/*`.
 * (Legacy name `google-sync` was removed; import from `@/lib/studio-api`.)
 */
...
export interface StoredInvoice {
  ...
  notes?: string;
  /** Songs chosen from the repertoire library for this booking */
  songSelections?: Array<{ songId: string; title?: string; artist?: string }>;
  /** Set when loaded from the database API */
  updatedAt?: string;
}
...
/** Returns true when the app can persist to the database (always, when deployed with DATABASE_URL). */
export const isStudioApiAvailable = (): boolean => true;

/** @deprecated Use `isStudioApiAvailable` */
export const isGoogleSyncEnabled = isStudioApiAvailable;
...
/** Persist a single invoice to Neon (`POST /api/invoices`). */
...
/** @deprecated Use `persistInvoice` */
export const saveInvoiceToGoogle = persistInvoice;

/** @deprecated Use `fetchInvoices` */
export const fetchInvoicesFromCloud = fetchInvoices;
...
export const saveConfigToGoogle = saveConfig;
